# n8n Google Drive 整合與自動搬移計畫

本計畫旨在優化 n8n 工作流，實現文章處理完後自動將雲端資料夾搬移至「已上稿」分頁，以保持母資料夾整潔。

## 建議變更

### n8n 工作流 (autotypein.json)

1. **尋找「已上稿」資料夾 (Find Uploaded Folder)**:
   - **節點類型**: HTTP Request
   - **邏輯**: 在目前的品牌母資料夾下，搜尋名稱完全等於「已上稿」的資料夾。
   - **目的**: 取得目標路徑的 `folderId`。

2. **搬移資料夾 (Move Folder)**:
   - **節點類型**: HTTP Request
   - **方法**: `PATCH`
   - **URL**: `https://www.googleapis.com/drive/v3/files/{{ 原始資料夾ID }}`
   - **參數**:
     - `addParents`: 剛才找到的「已上稿」資料夾 ID。
     - `removeParents`: 原始品牌母資料夾 ID。

### Google Sheets 自動化 (Apps Script)

- 維持 **v17 增量日期合併** 與 **核取方塊轉換** 邏輯。

## 驗證計畫

### 自動化測試

1. **n8n 完整測試**: 執行工作流，確認：
   - 資料成功寫入 Google Sheets。
   - 原始資料夾從母目錄消失。
   - 資料夾出現在該母目錄下的「已上稿」子目錄中。
2. **連結有效性測試**: 確認試算表中的「雲端連結」在搬移後依然能正常開啟。

### 手動驗證

- 若母資料夾下沒有「已上稿」資料夾，工作流將因找不到目標而報錯（此為預期行為，請使用者確保各品牌母目錄下皆有名為「已上稿」的資料夾）。
